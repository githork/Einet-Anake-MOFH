{if $tsAction == 'admin-noticia_edit'}
<form method="POST" name="edit-noticia" id="edit-noticia">
<input type="hidden" class="input" name="nid" value="{$tsNoticia.data.not_id}"/>
<input type="hidden" class="input" name="type" value="{$tsType}"/>
<section class="item">
<label>Message:</label>
<textarea class="input" id="not-msg" name="not-msg" maxlength="300" cols="30" rows="2">{$tsNoticia.data.not_body}</textarea>
<small>In the message field you are allowed to BBCode[]</small>
</section>
</form>
{elseif $tsAction == 'admin-noticia_update'}
{if $tsNoticias.data}
<!-- o -->
...
<img class="avatar" src="{$tsConfig.url}/files/perfiles/{$n.user_id}_50.jpg">
<span id="msg-right">
